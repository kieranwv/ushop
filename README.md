...
【前端模板】社区电商微信小程序，基于 Uniapp + uView UI 开发。模板基于成熟的产品设计及 UI 设计，同时提供商城流程、社区流程、积分、我的等 42 张全流程页面模板。全模块化开发，支持灵活分包及组件搭配组装。
...
💡 多端开发 - 基于 `uniapp` 的移动跨端技术，一套代码实现 `Android` / `IOS`、小程序、H5页面、轻应用等；

🗃️ 开箱即用 - 提供面向社区电商场景的 42 张页面模板，包含商城、订单、社区、用户、售后和积分商城等全流程页面；

🎨 主题颜色 - 将产品设计图中的主题色抽离，集成到一套主题模板中，可修改 `CSS Var` 变量，快速进行主题切换；

🧩 组件库 - 集成 Uniapp 成熟的组件库 — uView UI ，采用模块化开发，二次封装业务组件，维护性高、扩展性高；

🔑 分包策略 - 提供灵活的微信小程序分包策略，按照不同的功能模块进行代码解耦分离，同时抽离公共组件和方法；
